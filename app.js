...
class NumberToLetter {
    ...
    convertThreeDigitsArabic(n) {
        ...
        const hundredsAr = ["", "مئة", "مئتان", "ثلاثمئة", "أربعمئة", "خمسمئة", "ستمئة", "سبعمئة", "ثمانمئة", "تسعمئة"];
        ...
        let rem = n % 100;
        if (hundreds > 0) {
            parts.push(hundredsAr[hundreds]);
        }
        ...
        if (rem > 0) {
            if (rem < 10) {
                ...
                const unit = rem % 10;
                if (unit > 0) {
                    parts.push(unitsAr[unit] + " و" + tensAr[ten]);
                } else {
    ...
    convertArabic(num) {
        ...
        if (billions > 0) {
            if (billions === 1) {
                ...
                parts.push(this.convertThreeDigitsArabic(billions) + " مليارات");
            } else {
                parts.push(this.convertThreeDigitsArabic(billions) + " مليار");
            }
        }
        ...
        if (millions > 0) {
            if (millions === 1) {
                ...
                parts.push(this.convertThreeDigitsArabic(millions) + " ملايين");
            } else {
                parts.push(this.convertThreeDigitsArabic(millions) + " مليون");
            }
        }
        ...
        if (thousands > 0) {
            if (thousands === 1) {
                ...
                parts.push(this.convertThreeDigitsArabic(thousands) + " آلاف");
            } else {
                parts.push(this.convertThreeDigitsArabic(thousands) + " ألف");
            }
        }
        
        if (rem > 0) {
            parts.push(this.convertThreeDigitsArabic(rem));
        }
    ...
    toCurrencyWords(number, currency = "dinars", cents = "cts") {
        ...
        if (isAr) {
            let integerWords = this.convertArabic(integerPart);
            let result = `${integerWords} دينار جزائري`;
            if (decimalPart > 0) {
                let decimalWords = this.convertArabic(decimalPart);
                result += ` و ${decimalWords} سنتيم`;
            }
            return result;
        } else {
...
class Employee {
        ...
        if (isAr) {
            return `
                ...
                <head>
                    ...
                    <style>
                        body { font-family: 'Amiri', 'Scheherazade New', 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height: 1.8; color: #333; font-size: 11pt; background-color: #f9f9f9; margin: 0; padding: 0; direction: rtl; text-align: right; }
                        ...
                        .signature-block { margin-top: 60px; text-align: left; }
                ...
                <body>
                    <div class="container">
                        <header class="header">
                            <h3>${settings.name || 'اسم الشركة'}</h3>
                            <p>${settings.businessType || 'نوع النشاط'}</p>
                            <p>${settings.address || 'العنوان'}</p>
                            <p>رقم العضوية: ${settings.adherent || 'غير متوفر'}</p>
                        </header>
        
                        <h2 class="title-certif">شهادة عمل</h2>
        
                        <div class="body-content">
                            <p>نحن الموقعين أدناه، ${settings.managerName || '[اسم المدير]'}، مدير ${settings.name || '[اسم الشركة]'}، الكائن مقرها في: ${settings.address || '[عنوان الشركة]'}.</p>
                            
                            <p>نشهد بموجب هذه الشهادة أن السيد(ة): <strong>${this.name}</strong>، المولود(ة) بتاريخ <strong>${formatDate(this.birthDate)}</strong> في <strong>${this.birthPlace || '[مكان الميلاد]'}</strong>.</p>
                            
                            <p>والمقيم(ة) بـ: <strong>${this.address || '[عنوان الموظف]'}</strong></p>
                            
                            <p>رقم الضمان الاجتماعي: <strong>${this.nss || '[رقم الضمان الاجتماعي]'}</strong></p>
                            
                            <p>يعمل (تعمل) في مؤسستنا منذ تاريخ <strong>${formatDate(this.hireDate)}</strong> إلى يومنا هذا.</p>
                            
                            <p>بصفة: <strong>${this.position || '[المنصب]'}</strong></p>
                            
                            <p>وقد سلمت له/لها هذه الشهادة لاستعمالها في حدود ما يسمح به القانون.</p>
                        </div>
                        
                        <div class="signature-block">
                            <p class="signature-location">حرر بالجزائر في: ${today}</p>
    ...
    generateLeaveCertificateHTML(startDateStr, duration) {
        ...
        const isAr = settings.language === 'ar';
        const locale = isAr ? 'ar-DZ' : 'fr-FR';
        const formatLongDate = (date) => new Date(date).toLocaleDateString(locale, { day: '2-digit', month: 'long', year: 'numeric' });
        ...
        if (isAr) {
            return `
                ...
                <head>
                    <meta charset="UTF-8">
                    <title>شهادة إجازة - ${this.name}</title>
                     <style>
                        body { font-family: 'Amiri', 'Scheherazade New', 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; line-height: 1.8; color: #333; font-size: 11pt; background-color: #f9f9f9; margin: 0; padding: 0; direction: rtl; text-align: right; }
                        .container { max-width: 210mm; min-height: 297mm; margin: auto; padding: 2cm; background-color: white; box-sizing: border-box; display: flex; flex-direction: column; }
                        ...
                        .person-block { margin-bottom: 15px; display: grid; grid-template-columns: 120px 1fr; gap: 5px; }
                    ...
                    <div class="container">
                        <header class="header">
                            ...
                            <div class="header-info">
                                <span>رقم العضوية: ${settings.adherent || 'غير متوفر'}</span>
                                <span>رقم التعريف الضريبي: ${settings.nif || 'غير متوفر'}</span>
                                <span>رقم المادة: ${settings.art || 'غير متوفر'}</span>
                            </div>
                        </header>
                        
                        <h2 class="title-certif">شهادة إجازة</h2>
                        <p class="annee-conge">سنة ${anneeCongé}</p>
                        
                        <div class="content-section">
                            <div class="person-block">
                                <span class="label">السيد:</span><span>${settings.managerName || '[اسم المدير]'}</span>
                                <span class="label">الوظيفة:</span><span>مدير</span>
                            </div>
    
                            <p style="margin: 20px 0;"><strong>بناءً على الطلب المقدم من طرف:</strong></p>
    
                            <div class="person-block">
                                <span class="label">السيد(ة):</span><span>${this.name}</span>
                                <span class="label">الوظيفة:</span><span>${this.position}</span>
                            </div>
                            
                            <div class="decision-block">
                                <h3 class="decision-title">قرر ما يلي</h3>
                            </div>
    
                            <p class="conge-details">منح إجازة سنوية للاستجمام مدتها <strong>${duration}</strong> يوماً.</p>
    
                            <div class="date-details">
                                <span>تاريخ الخروج : ${formatLongDate(startDate)}</span>
                                <span>تاريخ العودة : ${formatLongDate(returnDate)}</span>
                            </div>
                        </div>
    
                        <div class="footer">
                            <p>حرر بالجزائر في ${formatLongDate(today)}</p>
                        </div>
    ...
    generateDischargeHTML(period) {
        ...
        const formatDate = (dateStr) => dateStr ? new Date(dateStr).toLocaleDateString(locale) : '';
        const formatLongDate = (date) => new Date(date).toLocaleDateString(locale, { day: '2-digit', month: 'long', year: 'numeric' });
        const today = new Date();
            ...
            return `
                ...
                <head>
                    ...
                    <style>
                        ...
                        .date-block { text-align: left; margin-top: 40px; }
                        @page { size: A4; margin: 0; }
                        @media print {
                            body { background-color: #fff; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
                            .container { box-shadow: none; border: none; margin: 0 auto; max-width: 100%; border-radius: 0; padding: 2cm; min-height: 0; height: auto; }
                ...
                <body>
                    <div class="container">
                        <header class="header">
                            <h2>${settings.name || 'اسم الشركة'}</h2>
                            <p>${settings.businessType || 'نوع النشاط'}</p>
                            <p>${settings.address || 'العنوان'}</p>
                        ...
                        <div class="body-content">
                            <p>${Translator.t('we_the_undersigned')}، <strong>${settings.managerName || '[اسم المدير]'}</strong>، مدير <strong>${settings.name || '[اسم الشركة]'}</strong>،
                            الكائن مقرها في ${settings.address || '[عنوان الشركة]'}، ${Translator.t('acting_as_employer')}.</p>
                            
                            <p>${Translator.t('declare_to_have_given')} السيد(ة) <strong>${this.name}</strong>، (المولود(ة) بتاريخ ${formatDate(this.birthDate)} في ${this.birthPlace || '[مكان الميلاد]'}، والمقيم(ة) بـ ${this.address || '[عنوان الموظف]' } )، العامل(ة) لدينا بصفة <strong>${this.position.toUpperCase()}</strong>.</p>
                            
                            <div class="amount-block">
                                <p>${Translator.t('net_amount_of')} <strong>${netToPayFormatted} دج</strong>،</p>
                        ...
                        <div class="date-block">
                            حرر بالجزائر في، ${formatLongDate(today)}
                        </div>
